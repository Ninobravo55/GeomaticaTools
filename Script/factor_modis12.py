...
def exportar_tabla_csv(ruta_csv, clases, lc_nombre):
    ...
    with open(ruta_csv, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["Valor", "R", "G", "B", "Color_Hex", "Descripcion", "LC_Type"])
...
"""
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        feedback.pushInfo(f"🗺  Clasificación seleccionada : {lc_sds} — {lc_nombre}")
        feedback.pushInfo(f"📋 Número de clases           : {len(clases_tabla) - 1} (+nodata)")
        feedback.pushInfo("")
        ...
        feedback.pushInfo(f"📄 Archivos MODIS 12 encontrados: {len(hdfs_validos)}")
        feedback.pushInfo("")
        ...
        for idx, (hdf, prefijo) in enumerate(hdfs_validos):
            ...
            if not sds_lista:
                feedback.reportError(f"   ⚠ No se pudieron leer los subdatasets de: {hdf}", False)
                continue

            feedback.pushInfo(f"   📋 Subdatasets disponibles ({len(sds_lista)}):")
            ...
            if os.path.exists(ruta_tif):
                ...
                feedback.pushInfo(f"   ➤ Exportando: {lc_sds}  →  {nombre_tif}")
                ok = procesar_banda(
                    sds_path     = sds_path,
                ...
                if ok:
                    ...
                    feedback.reportError(f"      ✘ Fallo al exportar {nombre_tif}", False)
                    continue
            ...
            try:
                ...
                feedback.reportError(f"      ⚠ Error exportando CSV: {e}", False)
            ...
            try:
                ...
                feedback.reportError(f"      ⚠ Error exportando QML: {e}", False)

        # ── Resumen final ────────────────────────────────────────────────────
        feedback.pushInfo("")
        feedback.pushInfo(f"{'='*60}")
        feedback.pushInfo(f"✅ MODIS 12 — {total_exportados} GeoTIFF exportado(s)")
