...
"""
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        feedback.pushInfo(f"📄 Archivos MODIS 11 encontrados: {len(hdfs_validos)}")
        feedback.pushInfo("")
        ...
        for idx, (hdf, prefijo) in enumerate(hdfs_validos):
            ...
            feedback.pushInfo(f"   🔢 Rango DN   : [{DN_MIN_LST}, {DN_MAX_LST}]")
            ...
            for nombre_sds, sufijo in bandas_activas:
                ...
                if sds_path is None:
                    feedback.pushInfo(f"   ⏭ '{nombre_sds}' no encontrado en SDS")
                    continue
            ...
            feedback.pushInfo(f"   ✅ {bandas_ok}/{len(bandas_activas)} banda(s) exportada(s)")
            total_exportados += bandas_ok

        feedback.pushInfo("")
        feedback.pushInfo(f"{'='*60}")
        feedback.pushInfo(f"✅ MODIS 11 — {total_exportados} archivo(s) exportado(s) en total")
