...
def get_mask_sentinel2_l2a(do_clouds, do_cirrus, do_shadows, do_snow, do_water):
    def _mask(image):
        import ee
...
def _aplicar_filtros(ee, collection, aoi, min_coverage, feedback):
    ...
    feedback.pushInfo(f"  Tras filtro geometrico (L1)       : {n_l1}  (−{n_orig - n_l1})")
...
class GEEDescargarIndices(GeomaticapeAlgorithm):
    ...
    def shortHelpString(self):
        catalogo = ''.join(
            f'<li><b>{k}</b> — <small>{INDICES[k]["name"]} | Formula: <code>{INDICES[k]["formula"]}</code></small></li>'
            for k in INDEX_KEYS
        )
        return f"""
...
"""
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        do_shadows   = self.parameterAsBool(parameters, self.MASK_SHADOWS, context)
        ...
        csv_path     = self.parameterAsFileOutput(parameters, self.OUTPUT_TABLE, context)
        ...
        if export_metodo == 1 and not gcs_bucket:
            raise QgsProcessingException("Debe especificar un Bucket de Google Cloud Storage.")
        if export_metodo == 2 and not local_dir:
            feedback.pushWarning("No se especificó una Carpeta local para la Descarga Directa. Cambiando a Google Drive por defecto.")
            export_metodo = 0

        # ── Validaciones basicas ──────────────────────────────────
        bands_needed = index_def['bands']
        missing_roles = [r for r in bands_needed if info['roles'].get(r) is None]
        if missing_roles:
            raise QgsProcessingException(
                f"El sensor seleccionado ({info['label']}) no posee las bandas necesarias "
                f"para calcular el indice {idx_code} (falta: {', '.join(missing_roles)})."
            )
        ...
        feedback.pushInfo(f"  Indice         : {idx_code} - {index_def['name']}")
        ...
        if export_metodo == 0:
            feedback.pushInfo(f"  Destino        : Google Drive ({GDRIVE_FOLDER})")
        elif export_metodo == 1:
            feedback.pushInfo(f"  Destino        : Google Cloud Storage ({gcs_bucket})")
        else:
        ...
        if info['tipo'] == 'landsat':
            mask_fn = get_mask_landsat(do_clouds, do_cirrus, do_shadows, do_snow, do_water)
        elif info['sensor'] == 'SENTINEL2_L2A':
            mask_fn = get_mask_sentinel2_l2a(do_clouds, do_cirrus, do_shadows, do_snow, do_water)
        else:
        ...
        def extract_meta(img):
            ...
            id_prod = img.get('LANDSAT_PRODUCT_ID') if info['tipo'] == 'landsat' else img.get('PRODUCT_ID')
        ...
        for i, feat in enumerate(meta_features):
            ...
            if info['tipo'] == 'landsat':
                ...
                for role in bands_needed:
                    ...
                    scaled_bands[role] = img.select(gee_band).multiply(0.0000275).add(-0.2)
            else:
            ...
            img_index = img.expression(gee_formula, scaled_bands).rename([idx_code]).clip(aoi)
            ...
            if export_metodo == 2: # Local
                ...
                try:
                    _descarga_directa(ee, img_index, info['scale_sr'], aoi, out_path, feedback)
                    task_id = 'LOCAL'
                    feedback.pushInfo(f"  [{i+1:03d}/{n_imagenes}] {idx_code:<5} → {nombre_index} (Descargado)")
                except Exception as e:
                    feedback.pushWarning(f"Descarga directa falló: {e}. Reintentando con Google Drive...")
            ...
            # ── Registro ──────────────────────────────────────────
            dest_str = "Local" if export_metodo == 2 else ("GCS" if export_metodo == 1 else GDRIVE_FOLDER)
            registros.append({
                'N'            : i + 1,
