...
def _gram_schmidt(pan_arr, ms_arr, weights=None, nodata=None):
    ...
    if weights and len(weights) == n_bands:
        ...
        s = w.sum()
        w = w / s if s > 1e-10 else np.ones(n_bands, dtype=np.float64) / n_bands
    else:
...
def _write_output_geotiff(out_path, bands_arr, ms_names, pan_ds,
    ...
    if ds_out is None:
        raise RuntimeError(f"No se pudo crear el GeoTIFF de salida: {out_path}")
...
class LandsatPansharpening(GeomaticapeAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr("Herramienta interactiva de Pansharpening. Úsela desde el menú.")
