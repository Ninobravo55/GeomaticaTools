...
FACTOR      = 0.0001
OFFSET      = 0.0
NODATA_OUT  = -0.3       # valor nodata en reflectancia (-0.3 está fuera del rango físico)
DTYPE_OUT   = "float32"
RESAMPLE_NN = False
...
"""
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        feedback.pushInfo(f"📄 Archivos MODIS 13 encontrados: {len(hdfs_validos)}")
        feedback.pushInfo(f"🔢 Rango DN válido              : [{DN_MIN_VI}, {DN_MAX_VI}]")
        feedback.pushInfo("")
        ...
        for idx, (hdf, prefijo) in enumerate(hdfs_validos):
            ...
            feedback.pushInfo(f"   ✅ {bandas_ok}/2 índice(s) exportado(s) [NDVI, EVI]")
            total_exportados += bandas_ok

        feedback.pushInfo("")
        feedback.pushInfo(f"{'='*60}")
        feedback.pushInfo(f"✅ MODIS 13 — {total_exportados} archivo(s) exportado(s) en total")
