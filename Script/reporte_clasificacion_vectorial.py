...
"""
    ...
    def processAlgorithm(self, parameters, context, feedback):
        try:
            ...
            raise QgsProcessingException("No se encontro la libreria matplotlib. Instala matplotlib para generar graficos.")

        vl = self.parameterAsVectorLayer(parameters, self.INPUT_VECTOR, context)
        ...
        if not stats:
            raise QgsProcessingException("No se encontraron registros validos para agrupar.")

        # Calcular totales y porcentajes
        total_area = sum(stats.values())
        clases = list(stats.keys())
        areas = [stats[c] for c in clases]
        porcentajes = [(a / total_area) * 100 if total_area > 0 else 0 for a in areas]
