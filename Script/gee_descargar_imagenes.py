...
def _make_mask_sentinel2(mask_opts, is_l2a):
    ...
    def _mask(image):
        ...
        if is_l2a:
            ...
            if use_clouds:
                conditions.append(scl.neq(8).And(scl.neq(9))) # Cloud Medium / High
            if use_cirrus:
...
"""
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterBoolean(
            self.MASK_SNOW_ICE,
            self.tr('Enmascarar nieve/hielo (QA_PIXEL bit 5 / SCL=11 en Sentinel-2 L2A)'),
            defaultValue=False,
            optional=True
        ))
        self.addParameter(QgsProcessingParameterBoolean(
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        mask_clouds   = self.parameterAsBool(parameters, self.MASK_CLOUDS,   context)
        mask_cirrus   = self.parameterAsBool(parameters, self.MASK_CIRRUS,   context)
        mask_shadows  = self.parameterAsBool(parameters, self.MASK_SHADOWS,  context)
        mask_snow_ice = self.parameterAsBool(parameters, self.MASK_SNOW_ICE, context)
        ...
        feedback.pushInfo(
            f"  Mascaras       : "
            + (', '.join(mascaras_activas) if mascaras_activas else 'Ninguna (todas desactivadas)')
        )
        feedback.pushInfo(f"  Bbox WGS84     : [{lon_min:.5f}, {lat_min:.5f}, "
                          f"{lon_max:.5f}, {lat_max:.5f}]")
        feedback.pushInfo(f"  Google Drive   : {info['gdrive']}")
        if export_metodo == 0:
            feedback.pushInfo(f"  Destino        : Google Drive ({info['gdrive']})")
        ...
        for i, feat in enumerate(meta_features):
            ...
            if export_metodo == 2: # Local
                ...
                try:
                    _descarga_directa(ee, img_sr, info['scale_sr'], aoi, out_path, feedback)
                    task_id_sr = 'LOCAL'
                    feedback.pushInfo(f"  [{i+1:03d}/{n_imagenes}] SR   → {nombre_sr} (Descargado)")
                except Exception as e:
            ...
            if con_tirs and info['bandas_tirs']:
                ...
                if export_metodo == 2:
                    ...
                    try:
                        _descarga_directa(ee, img_tirs, info['scale_tirs'], aoi, out_tirs, feedback)
                        task_id_tirs = 'LOCAL'
                        feedback.pushInfo(f"  [{i+1:03d}/{n_imagenes}] TIRS → {nombre_tirs} (Descargado)")
                    except Exception as e:
                        feedback.pushWarning(f"Descarga directa falló: {e}. Reintentando con Google Drive...")
            ...
            # ── Registro ──────────────────────────────────────────
            dest_str = "Local" if export_metodo == 2 else ("GCS" if export_metodo == 1 else info['gdrive'])
            registros.append({
                'N'            : i + 1,
