...
def _parametros_por_banda(sds_nombre, prefijo):
    ...
    # Identificar bandas de calidad, incertidumbre, días, o ángulos que no usan escala
    if "qa" in n or "quality" in n or "uncertainty" in n or "local" in n or "day" in n or "snow" in n or "obs" in n:
        return 1.0, 0.0, -32768, "int16", None, None  # Sin factor de escala, sin rango válido
    else:
...
class FactorMODIS43(GeomaticapeAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        self.addParameter(QgsProcessingParameterFile(
            self.INPUT_FOLDER,
            self.tr("Carpeta con archivos MODIS 43 HDF\n(MCD43A1, MCD43A2, MCD43A3, MCD43A4)"),
            behavior=QgsProcessingParameterFile.Folder
        ))
        self.addParameter(QgsProcessingParameterFolderDestination(
            self.OUTPUT_FOLDER,
            self.tr("Carpeta raíz de salida GeoTIFF (se crean subcarpetas por producto)")
        ))
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if not hdfs:
            raise Exception(self.tr("No se encontraron archivos .hdf en la carpeta."))
        ...
        if not hdfs_validos:
            raise Exception(self.tr("No se detectó ningún producto MODIS 43 válido."))

        feedback.pushInfo(self.tr("📄 Archivos MODIS 43 encontrados: {}").format(len(hdfs_validos)))
        feedback.pushInfo("")
        ...
        for idx, (hdf, prefijo) in enumerate(hdfs_validos):
            ...
            if not sds_lista:
                feedback.reportError(self.tr("   ⚠ No se pudieron leer los subdatasets"), False)
                continue
            ...
            for sds_path, sds_desc in sds_lista:
                ...
                if os.path.exists(ruta_tif):
                    feedback.pushInfo(self.tr("   ⏩ Ya existe: {}").format(nombre_tif))
                    bandas_ok += 1
                    if prefijo == "MCD43A4" and band_key:
                        mcd43a4_tifs[band_key] = ruta_tif
                    continue

                fct, off, nodata_o, dtype_o, dmin, dmax = _parametros_por_banda(nombre_sds, prefijo)
                
                feedback.pushInfo(self.tr("   ➤ {}  →  factor: {}").format(nombre_sds, fct))
                ...
                if ok:
                    ...
                    feedback.reportError(self.tr("      ✘ Fallo al exportar {}").format(nombre_tif), False)

            # Si es MCD43A4, hacer composición
            if prefijo == "MCD43A4" and len(mcd43a4_tifs) == 7:
                feedback.pushInfo(self.tr("   🛰 Generando composición de bandas MCD43A4..."))
                order = ["blue", "green", "red", "nir", "swir1", "swir2", "swir3"]
                tifs_to_merge = [mcd43a4_tifs[k] for k in order if k in mcd43a4_tifs]
                
                comp_tif = os.path.join(subcarpeta, f"{nombre_base}_Composicion.tif")
                if not os.path.exists(comp_tif):
                    ...
                    try:
                        ...
                        feedback.pushInfo(f"      ⚠ Error al asignar nombres a las bandas: {e}")
                    
                    feedback.pushInfo(self.tr("      ✔ Composición generada: {}").format(os.path.basename(comp_tif)))
                else:
                    feedback.pushInfo(self.tr("   ⏩ Ya existe composición: {}").format(os.path.basename(comp_tif)))
                
                # Eliminar las bandas individuales para MCD43A4 para que el dato de salida sea la composición
            ...
            feedback.pushInfo(self.tr("   ✅ {} banda(s) / composición exportada(s)").format(bandas_ok))
            total_exportados += bandas_ok

        feedback.pushInfo("")
        feedback.pushInfo(f"{'='*60}")
        feedback.pushInfo(self.tr("✅ MODIS 43 — {} archivo(s) exportado(s) en total").format(total_exportados))
