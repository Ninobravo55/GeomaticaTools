...
def _show_missing_library(lib_name, pip_cmd):
    def show():
        ...
        msg.setText(f"Se requiere instalar '{lib_name}' para usar este algoritmo.")
        msg.setInformativeText(f"Por favor, ve al menú GeomaticaPe -> 'Instalar dependencias de Python...' para instalar las librerías necesarias.\n\nComando manual:\n{pip_cmd}")
        if iface:
            msg.setParent(iface.mainWindow(), msg.windowFlags())
        (msg.exec_() if hasattr(msg, 'exec_') else msg.exec())
    QTimer.singleShot(0, show)
    raise QgsProcessingException(f"Librería requerida no encontrada: {lib_name}. Por favor use 'Instalar dependencias de Python' en el menú.")
...
def _rasterizar_roi(roi_path, info, campo_id, dst_path):
    ...
    out = drv.Create(dst_path, info["cols"], info["rows"], 1, gdal.GDT_Int32, options=["COMPRESS=LZW", "TILED=YES", "BIGTIFF=IF_SAFER"])
...
def _plot_matriz_b64(M, clases, oa, kappa):
    try:
        ...
        return ""
    fig, ax = plt.subplots(figsize=(max(6, len(clases)*0.8), max(5, len(clases)*0.8)))
...
def _plot_heatmap_b64(report_dict, clases_lbl, id2cls, global_clases_id):
    ...
    for cls_id, cls_lbl in zip(global_clases_id, clases_lbl):
        ...
        key = next((k for k in candidates if k in report_dict and isinstance(report_dict[k], dict)), None)
        ...
        if key:
            ...
            # Clase no encontrada en el reporte: añadir fila de ceros para no perder la clase
    ...
    data = np.array(data)
    fig, ax = plt.subplots(figsize=(max(5, len(valid_classes)*0.6), max(4, len(valid_classes)*0.5)))
...
def _plot_importance_b64(importances, band_names):
    ...
    fig, ax = plt.subplots(figsize=(max(6, len(band_names)*0.5), max(4, len(band_names)*0.4)))
...
def _tabla_html_metricas_por_clase(report_dict, id2cls, global_clases_id, M_final):
    ...
    for i, cls_id in enumerate(global_clases_id):
        ...
        def color_cell(v):
            if v >= 0.90: bg, fg = "#1a7a4a", "white"
            elif v >= 0.80: bg, fg = "#2E8B57", "white"
            elif v >= 0.70: bg, fg = "#85bb9a", "#1a3a2a"
            elif v >= 0.60: bg, fg = "#f0b429", "#3a2a00"
            else: bg, fg = "#d94f4f", "white"
...
def _generate_html_report(out_dir, method_name, val_method_name, metrics_dict, M, clases_lbl,
    ...
    tabla_clases_rows  = _tabla_html_metricas_por_clase(report_dict, id2cls, global_clases_id, M)
    tabla_imp_rows     = _tabla_html_importancia(importances, band_names)
    n_clases           = len(global_clases_id)
    clases_nombres     = ", ".join([id2cls.get(int(c), str(c)) for c in global_clases_id])
...
"""
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterNumber(
            self.PCT_VAL, self.tr("% de Validacion (solo para Hold-Out y Monte Carlo)"),
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if method_name == "Gaussian Mixture Model (GMM)":
            ...
            class GMMClassifier:
                ...
                def fit(self, X, y):
                    ...
                    for c in self.classes:
                        Xc = X[y == c]
                        gmm = GaussianMixture(n_components=1, random_state=self.seed)
                        if len(Xc) > 0: gmm.fit(Xc)
                        self.models[c] = gmm
                    return self
                def predict(self, X):
            ...
            model_factory = lambda s: GMMClassifier(s)
            
        elif method_name == "Random Forest (RF)":
            from sklearn.ensemble import RandomForestClassifier
            model_factory = lambda s: RandomForestClassifier(n_estimators=100, n_jobs=-1, random_state=s)
            
        elif method_name == "Support Vector Machine (SVM)":
            from sklearn.svm import SVC
            model_factory = lambda s: SVC(random_state=s, probability=False)
            
        elif method_name == "K-Nearest Neighbors (KNN)":
            from sklearn.neighbors import KNeighborsClassifier
            model_factory = lambda s: KNeighborsClassifier(n_neighbors=5, n_jobs=-1)
            ...
            class XGBWrapper:
                ...
                def fit(self, X, y):
                    ...
                    self.model = xgb.XGBClassifier(random_state=self.seed, eval_metric='mlogloss', n_jobs=-1)
                    self.model.fit(X, y_enc)
                    return self
                def predict(self, X):
                    preds = self.model.predict(X)
                    return self.le.inverse_transform(preds)
            model_factory = lambda s: XGBWrapper(s)
            ...
            class CBWrapper:
                ...
                def fit(self, X, y):
                    import catboost as cb
                    from sklearn.preprocessing import LabelEncoder
                    # LabelEncoder para manejar clases no numéricas o no contiguas
                    self.le = LabelEncoder()
                    y_enc = self.le.fit_transform(y)
                    self.model = cb.CatBoostClassifier(random_seed=self.seed, verbose=False, thread_count=-1)
                    self.model.fit(X, y_enc)
                    return self
                def predict(self, X):
                    preds = self.model.predict(X)
            ...
            model_factory = lambda s: CBWrapper(s)
            
        elif method_name == "Extra Trees (ET)":
            from sklearn.ensemble import ExtraTreesClassifier
            model_factory = lambda s: ExtraTreesClassifier(n_estimators=100, n_jobs=-1, random_state=s)
            
        elif method_name == "Gradient Boosting Classifier (GBC)":
            from sklearn.ensemble import GradientBoostingClassifier
            model_factory = lambda s: GradientBoostingClassifier(random_state=s)
            
        elif method_name == "Logistic Regression (LR)":
            from sklearn.linear_model import LogisticRegression
            model_factory = lambda s: LogisticRegression(random_state=s, max_iter=1000, n_jobs=-1)
            
        elif method_name == "Gaussian Naive Bayes (NB)":
            from sklearn.naive_bayes import GaussianNB
            model_factory = lambda s: GaussianNB()
            
        elif method_name == "Multi-layer Perceptron (MLP)":
            from sklearn.neural_network import MLPClassifier
            model_factory = lambda s: MLPClassifier(hidden_layer_sizes=(100, 50), max_iter=500, random_state=s)
        ...
        # Crear shape temporal y añadir campo especial __FID__ para LOOCV por Poligono
        tmp_roi = os.path.join(out_dir, "_tmp_roi_proj.shp")
        opts = QgsVectorFileWriter.SaveVectorOptions()
        opts.driverName = "ESRI Shapefile"
        opts.fileEncoding = "UTF-8"
        QgsVectorFileWriter.writeAsVectorFormatV3(roi_layer, tmp_roi, context.transformContext(), opts)
        ...
        if n_train_total == 0:
            raise QgsProcessingException("No se obtuvieron pixeles del ROI válidos.")
        ...
        for yoff in range(0, rows, chunk_size):
            ...
            for xoff in range(0, cols, chunk_size):
                ...
                for b in range(1, info["nbands"] + 1):
                    arr = ds.GetRasterBand(b).ReadAsArray(xoff, yoff, xsize, ysize).astype(np.float32)
                    nd = info["nodata"][b - 1]
                    if nd is not None:
                        arr[arr == nd] = np.nan
                ...
                xgeo_c = gt[0] + (cols_c + xoff + 0.5) * gt[1] + (rows_c + yoff + 0.5) * gt[2]
                ygeo_c = gt[3] + (cols_c + xoff + 0.5) * gt[4] + (rows_c + yoff + 0.5) * gt[5]
        ...
        if not X_list:
            raise QgsProcessingException("No se obtuvieron pixeles validos despues de filtrar NaNs.")
        ...
        if val_method_name == "Hold-Out (70/30, 80/20...)":
            try:
                tr_i, te_i = train_test_split(np.arange(len(y_full)), test_size=pct/100.0, random_state=seed, stratify=y_full)
            except ValueError:
                tr_i, te_i = train_test_split(np.arange(len(y_full)), test_size=pct/100.0, random_state=seed)
            ...
            skf = StratifiedKFold(n_splits=k_val, shuffle=True, random_state=seed)
            try:
                cv_splits = list(skf.split(X_full, y_full))
            except ValueError:
                feedback.pushInfo("Aviso: No hay suficientes muestras de alguna clase para Stratified K-Fold. Usando K-Fold regular.")
                kf = KFold(n_splits=k_val, shuffle=True, random_state=seed)
                cv_splits = list(kf.split(X_full))
            ...
            # Para evitar MemoryError o demoras infinitas, limitaremos a máx combinaciones
            try:
                cv_splits = list(lpgo.split(X_full, y_full, fid_full))
                if len(cv_splits) > 500:
                    feedback.pushInfo(f"Aviso: Leave-P-Out generó {len(cv_splits)} iteraciones. Limitando a 500 aleatorias.")
                    import random
                    random.seed(seed)
                    cv_splits = random.sample(cv_splits, 500)
            ...
            ss = ShuffleSplit(n_splits=k_val, test_size=pct/100.0, random_state=seed)
            cv_splits = list(ss.split(X_full))

        if not cv_splits:
            raise QgsProcessingException("La metodologia seleccionada no genero particiones de validacion validas.")

        feedback.pushInfo(f"Total de iteraciones de validacion (Pliegues): {len(cv_splits)}")
        ...
        feedback.pushInfo(
            f"Shapefiles roi_train / roi_test: generados con el fold 1 de {len(cv_splits)} "
        ...
        _escribir_shp_puntos(train_shp, gt, info["proj"], list(_filas(X_tr_ex, y_tr_ex, xy_tr_ex)), cols_attr, tipos_attr)
        _escribir_shp_puntos(test_shp, gt, info["proj"], list(_filas(X_te_ex, y_te_ex, xy_te_ex)), cols_attr, tipos_attr)
        feedback.setProgress(30)

        # Entrenar iterativamente
        feedback.pushInfo(f"Iniciando ciclo de entrenamiento y validación ({method_name})...")
        confusion_matrices = []
        all_y_true = []
        all_y_pred = []
        ...
        for fold, (tr_idx, te_idx) in enumerate(cv_splits, 1):
            if fold % 5 == 0 or fold == len(cv_splits) or fold == 1:
                feedback.pushInfo(f" -> Procesando iteracion {fold}/{len(cv_splits)}...")
        ...
        clases_lbl = [f"{c} - {id2cls.get(int(c), str(c))}" for c in global_clases_id]
        
        oa = accuracy_score(all_y_true, all_y_pred)
        kappa = float(_kappa(np.sum(confusion_matrices, axis=0))) # Kappa is better calculated on the sum
        ...
        # classification_report primero — report_dict lo necesitan CSV 3, heatmap y reporte HTML
        ...
        img_heatmap_b64 = _plot_heatmap_b64(report_dict, clases_lbl, id2cls, global_clases_id)
        ...
        feedback.pushInfo("Entrenando modelo final y calculando Importancia de Variables...")
        ...
        with open(ruta_csv_imp, "w", newline="", encoding="utf-8") as fh:
            w = csv.writer(fh)
            w.writerow(["Rango", "Banda_Variable", "Importancia_Raw", "Importancia_Pct"])
        ...
        try:
            _logo_candidates = [
                os.path.join(os.path.dirname(__file__), "..", "Icons", "logo_geomatica.png"),
        ...
        for yoff in range(0, h, chunk_size):
            ...
            for xoff in range(0, w, chunk_size):
                ...
                if ok_all.any():
                    pred_valid = final_model.predict(flat[ok_all]).astype(np.int32)
                ...
                if chunk_count % max(1, total_chunks // 10) == 0:
                    feedback.setProgress(70 + int(25 * chunk_count / total_chunks))
        ...
        ds_vec = ogr.GetDriverByName("ESRI Shapefile").CreateDataSource(ruta_vec)
