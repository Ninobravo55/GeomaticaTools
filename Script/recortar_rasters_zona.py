...
def _dissolve_and_buffer_vector(vector_path, layer_name, buffer_m, feedback):
    ...
    if union_geom is None:
        raise RuntimeError("No se encontraron geometrias validas en el vector.")

    # Buffer (si > 0)
    if buffer_m and abs(buffer_m) > 1e-10:
        union_geom = union_geom.Buffer(buffer_m)
        if union_geom is None:
            raise RuntimeError("El buffer fallo (posible problema de CRS/unidades).")
...
def _raster_mask_to_vector(mask_path, buffer_m, feedback):
    ...
    if src_ds is None:
        raise RuntimeError(f"GDAL no pudo abrir la mascara raster: {mask_path}")
...
def _prepare_cutline(mask_path, mask_is_raster, mask_layer_name,
    ...
    """
    if mask_is_raster:
        cut_path, cut_layer = _raster_mask_to_vector(mask_path, buffer_m, feedback)
...
def ejecutar_recorte_simple(raster_path, mask_path, mask_is_raster,
    ...
    src_ds = None
    feedback.pushInfo(f"Bandas del raster fuente: {nb}  [{', '.join(band_names)}]")
    ...
    if crop_to_cutline:
        ...
        if ds_vec:
            lyr = ds_vec.GetLayerByName(cut_layer) if cut_layer else ds_vec.GetLayer(0)
            if lyr:
...
class RecortarRastersZonaDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self.cb_mask.setFilters(
            QgsMapLayerProxyModel.RasterLayer | QgsMapLayerProxyModel.PolygonLayer
        )
        ...
        nd_w = QWidget()
        nh = QHBoxLayout(nd_w)
        nh.setContentsMargins(0, 0, 0, 0); nh.setSpacing(4)
...
class RecortarRastersZona(GeomaticapeAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr("Herramienta interactiva para recortar rasters. Úsela desde el menú.")
