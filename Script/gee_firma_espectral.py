...
def _exportar_imagen(ee, imagen, nombre, destino, escala, aoi, metodo, feedback):
...
"""
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterNumber(
            self.MIN_COVERAGE,
            self.tr('Cobertura minima de pixeles validos en el AOI [0.0 - 1.0]'),
            type=QgsProcessingParameterNumber.Type.Double,
            defaultValue=0.80,
            minValue=0.0,
            maxValue=1.0,
        ...
        self.addParameter(QgsProcessingParameterEnum(
            ...
            options=['Google Drive', 'Google Cloud Storage (GCS)', 'Descarga Directa (imágenes pequeñas)'],
            defaultValue=0
        ))
        self.addParameter(QgsProcessingParameterString(
            self.GCS_BUCKET,
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        fecha_inicio = self.parameterAsString(parameters, self.DATE_START,   context).strip()
        fecha_fin    = self.parameterAsString(parameters, self.DATE_END,     context).strip()
        cloud_pct    = self.parameterAsDouble(parameters, self.CLOUD_PCT,    context)
        min_cov      = self.parameterAsDouble(parameters, self.MIN_COVERAGE, context)
        exp_drive    = self.parameterAsBool(parameters,   self.EXPORT_DRIVE, context)
        export_metodo = self.parameterAsEnum(parameters, self.EXPORT_METHOD, context)
        gcs_bucket   = self.parameterAsString(parameters, self.GCS_BUCKET, context).strip()
        ...
        if exp_drive:
            if export_metodo == 1 and not gcs_bucket:
                raise QgsProcessingException("Debe especificar un Bucket de Google Cloud Storage para exportar.")
            if export_metodo == 2 and not local_dir:
                raise QgsProcessingException("Debe especificar una Carpeta local de destino para la Descarga Directa.")
        ...
        feedback.pushInfo(f"📍 Extrayendo y reproyectando geometrias de muestreo ({geom_type_name})...")
        muestras = []   # lista de (geojson_dict, clase, id_obj)
        crs_capa  = puntos_layer.crs()
        crs_wgs84 = QgsCoordinateReferenceSystem("EPSG:4326")
        transform = QgsCoordinateTransform(crs_capa, crs_wgs84, QgsProject.instance())

        for idx_f, feature in enumerate(puntos_layer.getFeatures()):
            geom = feature.geometry()
            if geom.isNull():
                continue
            # Clonar y reproyectar la geometria completa (funciona para puntos Y poligonos)
        ...
        if not muestras:
            raise QgsProcessingException("La capa de muestreo no contiene ninguna geometria valida.")

        feedback.pushInfo(f"  -> Se obtuvieron {len(muestras)} objeto(s) valido(s) para el analisis.")
        ...
        if not crs_ext.isGeographic():
            tr = QgsCoordinateTransform(crs_ext, crs_wgs84, QgsProject.instance())
        ...
        feedback.pushInfo(f"🔍 Buscando la escena satelital de menor nubosidad...")
        ...
        # Ordenar por nubosidad (menor porcentaje primero) y seleccionar la primera
        ...
        fecha_adq = ee.Date(image_raw.get('system:time_start')).format('YYYY-MM-dd').getInfo()
        ...
        # Convertir geometrias QGIS (puntos o poligonos) en FeatureCollection GEE
        ee_features = []
        ...
        feedback.pushInfo("📥 Descargando datos espectrales a la maquina local...")
        ...
        for feat_gee in sampled_data['features']:
            ...
            # Verificar que todos los valores de banda esten presentes (no enmascarados)
            valido = True
            for band_idx, gb in enumerate(gee_bands):
                # reduceRegions con mean() devuelve el nombre de la banda como clave
        ...
        if n_omitidos:
            feedback.pushInfo(f"  -> {n_omitidos} objeto(s) omitido(s) por mascara de nubes o sombras.")
        ...
        feedback.pushInfo(f"  -> Extraccion exitosa para {len(df)} objeto(s) de muestreo.")
        ...
        # ── Exportar a Excel ──────────────────────────────────────
        feedback.pushInfo(f"💾 Guardando reporte Excel: {os.path.basename(excel_out)}")
        ...
        ax.set_xlabel('Longitud de onda (μm)', fontsize=11, fontweight='bold', labelpad=8)
        ax.set_ylabel('Reflectancia (0.0 - 1.0)', fontsize=11, fontweight='bold', labelpad=8)
        ax.set_title(info['titulo'] + f"\nEscena: {id_producto} ({fecha_adq})", fontsize=12, fontweight='bold', pad=15)
        
        ax.grid(True, linestyle='--', alpha=0.5)
        ax.legend(handles=leyenda_patches, loc='upper right', frameon=True, facecolor='white', framealpha=0.9)
        ...
        plt.close()
        feedback.pushInfo(f"  -> Grafico guardado en: {os.path.basename(grafico_out)}")

        # ── Exportar a Drive / GCS / Local (opcional) ───────────────────────────
        if exp_drive:
            feedback.pushInfo(f"💾 Iniciando exportación de la escena multiespectral completa...")
            # Limpiar ID
            id_clean = _limpiar_nombre(str(id_producto).replace('-', '_').replace('/', '_'))
            nombre_export = _limpiar_nombre(f"{fecha_adq[:4]}_{info['sensor']}_MULTISPECTRAL_{id_clean}")
            
            if export_metodo == 2: # Local
                total_px = _estimar_pixeles(lon_min, lat_min, lon_max, lat_max, info['scale_sr'])
                if total_px > LIMITE_DIRECTO_MP:
                    feedback.pushWarning(
                        f"AVISO: El área ({total_px/1e6:.1f} MP) puede superar el límite "
                        f"de descarga directa de GEE (~48 MB). Si falla, cambia a Drive o GCS."
                    )
                out_path = os.path.join(local_dir, f"{nombre_export}.tif")
                _descarga_directa(ee, image_raw.select(gee_bands).clip(aoi), info['scale_sr'], aoi, out_path, feedback)
                feedback.pushInfo(f"  -> Escena exportada localmente en: {out_path}")
            else: # Drive o GCS
                ...
                feedback.pushInfo(f"  -> Tarea de exportacion iniciada en GEE. Task ID: {task_id}")
