...
class GEEAuthTask(QgsTask):
    ...
    """
    def __init__(self, project_id, description="Autenticando Google Earth Engine"):
    ...
    def run(self):
        try:
            ...
            # Si se proporcionó un proyecto, intentamos inicializar para comprobar
            kwargs = {}
            if self.project_id:
            ...
            QgsMessageLog.logMessage(f"Error GEE Auth: {traceback.format_exc()}", "GeomaticaPe", Qgis.Critical)
            return False

    def finished(self, result):
        if result and self.success:
            QgsMessageLog.logMessage("GEE Autenticación Exitosa", "GeomaticaPe", Qgis.Success)
            ...
            QgsMessageLog.logMessage(f"Fallo GEE Auth: {self.exception_msg}", "GeomaticaPe", Qgis.Critical)
...
class GEEAuthDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        current_project = settings.value('geomaticape/gee_project', '', type=str)
        ...
        self.btn_auth = QPushButton(self.tr("Autenticar con Google y verificar conexión"))
    ...
    def save_config(self):
        ...
        QMessageBox.information(self, self.tr("Guardado"), self.tr("Configuración guardada exitosamente."))
        self.accept()
    ...
    def start_auth(self):
        try:
            ...
            QMessageBox.critical(
                self, 
                self.tr("Error"), 
                self.tr("No se encontró el módulo 'earthengine-api'. Instálalo primero.")
            )
            return
        ...
        # Configurar la tarea
        self.task = GEEAuthTask(project_id, self.tr("Autenticando Google Earth Engine"))
    ...
    def on_task_completed(self):
        ...
        # Guardar automáticamente la configuración si la autenticación fue exitosa
        self.save_config()
    ...
    def on_task_failed(self):
        ...
        self.btn_save.setEnabled(True)
        self.progress.hide()
        msg = self.task.exception_msg if self.task and hasattr(self.task, 'exception_msg') else "Error desconocido"
        QMessageBox.critical(
            self, 
