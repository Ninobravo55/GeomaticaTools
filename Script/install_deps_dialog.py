...
import os
import tempfile
from qgis.PyQt.QtWidgets import QDialog, QVBoxLayout, QTextEdit, QPushButton, QLabel, QMessageBox
...
class InstallDepsDialog(QDialog):
    def __init__(self, missing_deps, parent=None):
        ...
        self.info_label = QLabel(f"Instalando paquetes requeridos: <b>{', '.join(missing_deps)}</b><br>Por favor espera, no cierres esta ventana. El proceso puede tomar unos minutos dependiendo de la conexión a internet.")
        ...
        self.text_edit.setStyleSheet("background-color: #1e1e1e; color: #00FF00; font-family: 'Consolas', 'Courier New', monospace; font-size: 12px; padding: 5px;")
...
echo ========================================================
"""
            self.temp_bat = os.path.join(tempfile.gettempdir(), "install_geomaticape_deps.bat")
    ...
    def process_finished(self, exitCode, exitStatus):
        ...
        if exitCode == 0:
            ...
            self.info_label.setText("<b>La instalación terminó con errores.</b> Revisa la consola en color verde de abajo para más detalles.")
